...
const submitProfileEdit = async(onEvennt: FormSubmitEvent<ProfileSchema>) => {
    try {
        ...
        toast.add({title:'Profile updated', description:'Profile successfully updated.', color: 'success'});
        emit('profileUpdated');
    } catch (error) {
...
<template>
    <div class="h-full flex flex-col">
        <div class="space-y-4 flex-1 ">
            <UForm :schema="profileSchema" :state="profileState"
                class="flex flex-col flex-1 space-y-4" @submit="submitProfileEdit">
